...
#making jitclass object "solution"
spec = [
    ('perm1', int64[:]),
    ('perm2', int64[:]),
    ('cost', int64),
...
def evalDelta(sol, mv):
    ...
    for i in range(dim):
        #if i not in mv prevents jit-compilation
        ...
        for j in range(dim):
            ...
            # if j not in mv: #prevents jit-compilation
...
#parallel=True triggers prange loop to be parallelized
@njit(parallel=True)
def evalCosts(sol):
    costs = np.zeros(np.shape(moves)[0])
    for i in prange(np.shape(moves)[0]):
        costs[i] = evalDelta(sol, moves[i])
    return costs

#triggers autoparallelization of argmax?
@njit(parallel=True)
def localSearch(sol, maxiter):
    iter = 0
    for i in range(maxiter):
        costs = evalCosts(sol)  # in parallel!!
        bestind = np.argmax(costs)
        delta = costs[bestind]
