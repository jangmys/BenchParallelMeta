...
######################################
#parallel (multi-threaded) evaluation of neighbours
...
#
#number of threads is controlled by
...
if size(ARGS)[1]!=2
	println("Need 2 arguments (instance, batchsize), ex: julia ./ils_q3ap.jl nug12 1000")
	exit()
end
...
#run local search (="compilation")
sol=Solution(collect(1:dim),collect(1:dim),0)
elap2 = @elapsed localSearch!(sol, 100)
