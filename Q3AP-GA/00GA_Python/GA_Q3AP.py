...
# flow[i,k] * flow[i,k] * dist[j,n] * dist[p,q]
...
def evalDeltas(sol):
    cost_arr = np.zeros(len(moves),dtype=int)
    p1 = sol['perm1']
    p2 = sol['perm2']
...
def POSxover(par1, par2):
    ...
    for k in range(2):  # for both permutations
        flag = np.zeros(dim)
        # randomly choose between 3 and 9 positions
        pos = np.random.choice(np.arange(dim), np.random.randint(3, 9), False)
        # copy
...
def roulette():
    prob = [(100.0/p['cost'][0]) for p in pop]
